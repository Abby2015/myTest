	...
	var controller = {
			...
			init: function() {
				...
				if(openid_=="" || openid_==null || openid_=="null"){
					...
					location.href=amGloble.config.selfRoot + "pages/bnbjc1508/index.html?tag="+_tag;
				}else{
					_this.toTarget();
				}
			},
			...
			toTarget : function(){
	        	...
	        	if (amGloble.config.debug == true) alert("[-a01- tag]"+ tag_);
	        	var opt = {
	        	...
	        	amGloble.api.a01.post(opt, function (ret) {
	        		...
	        		if (ret.content.errCode != 0 || ret.content.data == null) {
	        			...
	        			if(ret.content.errCode="400025"){
	        				location.href=amGloble.config.selfRef + "pages/bnbjc1508/finish.html";
	        			}else if(ret.content.errCode="400029"){
	        				location.href=amGloble.config.selfRoot + "pages/bnbjc1508/index.html?tag="+tag_;
	        			}
	        		} else {
	        			...
	        			sessionStorage.setItem("isOtherDraw", ret.content.data.drawCoupon);
        				if (amGloble.config.debug == true) alert(amGloble.config.selfRef + "pages/bnbjc1508/index.html?tag="+ret.content.data.tag);
        				if (window._tag) { 
        	        		_tag.dcsMultiTrack("wt.event", "用户进入活动首页", "wt.msg", _this.getOpenid());
        	        	}
        				location.href=amGloble.config.selfRef + "pages/bnbjc1508/index.html?tag="+ret.content.data.tag;
	        		}
