...
public class Bjmls1609OpenMQConsumer extends BaseRabbitMQConsumer {
	private static final Logger log = LoggerFactory.getLogger(Bjmls1609OpenMQConsumer.class);
	
	private static final String QUEUE_NAME = Bjmls1609Open.class.getCanonicalName();
	
	private static final Bjmls1609Service service = SpringUtil.getBean(Bjmls1609Service.class);
	
	public Bjmls1609OpenMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.bjmls1609.open", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Bjmls1609Open open = JSON.parseObject(String.valueOf(msg.getPayload()), Bjmls1609Open.class);
