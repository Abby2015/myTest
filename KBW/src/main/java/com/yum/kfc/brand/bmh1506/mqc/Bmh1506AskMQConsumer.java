 ...
 *
 */
public class Bmh1506AskMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Bmh1506AskMQConsumer.class);
	
	private static final String QUEUE_NAME = Bmh1506Ask.class.getCanonicalName();
	
	private static final Bmh1506Service service = SpringUtil.getBean(Bmh1506Service.class);
	
	public Bmh1506AskMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.bmh1506.ask", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Bmh1506Ask ask = JSON.parseObject(String.valueOf(msg.getPayload()), Bmh1506Ask.class);
					service.saveAsk(ask);
