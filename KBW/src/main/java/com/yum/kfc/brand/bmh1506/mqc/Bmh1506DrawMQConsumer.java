 ...
 *
 */
public class Bmh1506DrawMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Bmh1506DrawMQConsumer.class);
	
	private static final String QUEUE_NAME = Bmh1506Draw.class.getCanonicalName();
	
	private static final Bmh1506Service service = SpringUtil.getBean(Bmh1506Service.class);
	
	public Bmh1506DrawMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.bmh1506.draw", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Bmh1506Draw draw = JSON.parseObject(String.valueOf(msg.getPayload()), Bmh1506Draw.class);
