 ...
 *
 */
public class Bmh1506OpenMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Bmh1506OpenMQConsumer.class);
	
	private static final String QUEUE_NAME = Bmh1506Open.class.getCanonicalName();
	
	private static final Bmh1506Service service = SpringUtil.getBean(Bmh1506Service.class);
	
	public Bmh1506OpenMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.bmh1506.open", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Bmh1506Open open = JSON.parseObject(String.valueOf(msg.getPayload()), Bmh1506Open.class);
