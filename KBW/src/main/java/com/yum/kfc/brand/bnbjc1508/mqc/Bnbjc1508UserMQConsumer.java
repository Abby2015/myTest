 ...
 *
 */
public class Bnbjc1508UserMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Bnbjc1508UserMQConsumer.class);
	
	private static final String QUEUE_NAME = Bnbjc1508User.class.getCanonicalName();
	
	private static final Bnbjc1508Service service = SpringUtil.getBean(Bnbjc1508Service.class);
	
	public Bnbjc1508UserMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.bnbjc1508.user", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Bnbjc1508User user = JSON.parseObject(String.valueOf(msg.getPayload()), Bnbjc1508User.class);
