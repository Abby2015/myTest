 ...
 *
 */
public class FooldayDrawRabbitMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(FooldayDrawRabbitMQConsumer.class);
	
	private static final String QUEUE_NAME = FooldayDraw.class.getCanonicalName();
	
	private static final FooldayService fooldayService = SpringUtil.getBean(FooldayService.class);
	
	public FooldayDrawRabbitMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.camp1504.draw", "1"));
	}
