 ...
 *
 */
public class Camp1505ShareMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Camp1505ShareMQConsumer.class);
	
	private static final String QUEUE_NAME = Camp1505Share.class.getCanonicalName();
	
	private static final Camp1505Service service = SpringUtil.getBean(Camp1505Service.class);
	
	public Camp1505ShareMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.camp1505.share", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Camp1505Share share = JSON.parseObject(String.valueOf(msg.getPayload()), Camp1505Share.class);
