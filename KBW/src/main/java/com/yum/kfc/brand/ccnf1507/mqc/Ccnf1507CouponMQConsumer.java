 ...
 *
 */
public class Ccnf1507CouponMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Ccnf1507CouponMQConsumer.class);
	
	private static final String QUEUE_NAME = Ccnf1507Coupon.class.getCanonicalName();
	
	private static final Ccnf1507Service service = SpringUtil.getBean(Ccnf1507Service.class);
	
	public Ccnf1507CouponMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.ccnf1507.coupon", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Ccnf1507Coupon coupon = JSON.parseObject(String.valueOf(msg.getPayload()), Ccnf1507Coupon.class);
