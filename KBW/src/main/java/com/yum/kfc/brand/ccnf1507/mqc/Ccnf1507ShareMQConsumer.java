 ...
 *
 */
public class Ccnf1507ShareMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Ccnf1507ShareMQConsumer.class);
	
	private static final String QUEUE_NAME = Ccnf1507Share.class.getCanonicalName();
	
	private static final Ccnf1507Service service = SpringUtil.getBean(Ccnf1507Service.class);
	
	public Ccnf1507ShareMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.ccnf1507.share", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Ccnf1507Share share = JSON.parseObject(String.valueOf(msg.getPayload()), Ccnf1507Share.class);
