...
@Component
public class Dbwy1506InitiatorTaskJob {
	
	private static Logger logger = LoggerFactory.getLogger(Dbwy1506InitiatorTaskJob.class);
	...
	@Value("${wechat.sendMsg.db.describe}")			private String dbDescribe;
	@Value("${wechat.sendMsg.wy.describe}")			private String wyDescribe;
	@Value("${wechat.sendMsg.tplData}")			private String ptlData;
	
	private Dbwy1506Service service;
	private Dbwy1506TagInfo tagInfo;
	...
	public boolean saveCoupon(String promoCode, String openId){
		//封装发起用户的半价券
		Dbwy1506Coupon initiatorCoupon = new Dbwy1506Coupon();
	...
	public FeiRuiResult callSendMsgService(String data, String wxName){
		...
		//post参数
		Map<String, String> postParams = new HashMap<String, String>();
		String askFriend = tagInfo.getAskType() == 0 ? "逗比青年" : "文艺青年";
		String describe = tagInfo.getAskType() == 0 ? dbDescribe : wyDescribe;
		String tpl_data = ptlData.replace("{0}", tagInfo.getUserId()).replace("{1}", data).replace("{2}", askFriend).replace("{3}", wxName).replace("{4}", describe);
	...
	public void setService(Dbwy1506Service service) {
		this.service = service;
	}

	public void setTagInfo(Dbwy1506TagInfo tagInfo) {
