 ...
 *
 */
public class Dbwy1506CouponMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Dbwy1506CouponMQConsumer.class);
	
	private static final String QUEUE_NAME = Dbwy1506Coupon.class.getCanonicalName();
	
	private static final Dbwy1506Service service = SpringUtil.getBean(Dbwy1506Service.class);
	
	public Dbwy1506CouponMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.dbwy1506.coupon", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Dbwy1506Coupon coupon = JSON.parseObject(String.valueOf(msg.getPayload()), Dbwy1506Coupon.class);
