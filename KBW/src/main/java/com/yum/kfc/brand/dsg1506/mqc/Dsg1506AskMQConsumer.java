 ...
 *
 */
public class Dsg1506AskMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Dsg1506AskMQConsumer.class);
	
	private static final String QUEUE_NAME = Dsg1506Ask.class.getCanonicalName();
	
	private static final Dsg1506Service service = SpringUtil.getBean(Dsg1506Service.class);
	
	public Dsg1506AskMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.dsg1506.ask", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Dsg1506Ask ask = JSON.parseObject(String.valueOf(msg.getPayload()), Dsg1506Ask.class);
					service.saveAsk(ask);
