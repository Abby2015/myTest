 ...
 *
 */
public class Dsg1506CouponMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Dsg1506CouponMQConsumer.class);
	
	private static final String QUEUE_NAME = Dsg1506Coupon.class.getCanonicalName();
	
	private static final Dsg1506Service service = SpringUtil.getBean(Dsg1506Service.class);
	
	public Dsg1506CouponMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.dsg1506.coupon", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Dsg1506Coupon coupon = JSON.parseObject(String.valueOf(msg.getPayload()), Dsg1506Coupon.class);
