 ...
 *
 */
public class Dsg1506ShareMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Dsg1506ShareMQConsumer.class);
	
	private static final String QUEUE_NAME = Dsg1506Share.class.getCanonicalName();
	
	private static final Dsg1506Service service = SpringUtil.getBean(Dsg1506Service.class);
	
	public Dsg1506ShareMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.dsg1506.share", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Dsg1506Share share = JSON.parseObject(String.valueOf(msg.getPayload()), Dsg1506Share.class);
