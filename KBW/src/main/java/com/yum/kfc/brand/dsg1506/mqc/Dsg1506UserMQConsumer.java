 ...
 *
 */
public class Dsg1506UserMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Dsg1506UserMQConsumer.class);
	
	private static final String QUEUE_NAME = Dsg1506User.class.getCanonicalName();
	
	private static final Dsg1506Service service = SpringUtil.getBean(Dsg1506Service.class);
	
	public Dsg1506UserMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.dsg1506.user", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Dsg1506User user = JSON.parseObject(String.valueOf(msg.getPayload()), Dsg1506User.class);
