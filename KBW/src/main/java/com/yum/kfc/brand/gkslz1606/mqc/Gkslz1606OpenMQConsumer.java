...
public class Gkslz1606OpenMQConsumer extends BaseRabbitMQConsumer {
	private static final Logger log = LoggerFactory.getLogger(Gkslz1606OpenMQConsumer.class);
	
	private static final String QUEUE_NAME = Gkslz1606Open.class.getCanonicalName();
	
	private static final Gkslz1606Service service = SpringUtil.getBean(Gkslz1606Service.class);
	
	public Gkslz1606OpenMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.gkslz1606.open", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Gkslz1606Open open = JSON.parseObject(String.valueOf(msg.getPayload()), Gkslz1606Open.class);
