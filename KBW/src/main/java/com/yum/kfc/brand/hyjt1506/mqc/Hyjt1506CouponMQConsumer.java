 ...
 *
 */
public class Hyjt1506CouponMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Hyjt1506CouponMQConsumer.class);
	
	private static final String QUEUE_NAME = Hyjt1506Coupon.class.getCanonicalName();
	
	private static final Hyjt1506Service service = SpringUtil.getBean(Hyjt1506Service.class);
	
	public Hyjt1506CouponMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.hyjt1506.coupon", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Hyjt1506Coupon coupon = JSON.parseObject(String.valueOf(msg.getPayload()), Hyjt1506Coupon.class);
