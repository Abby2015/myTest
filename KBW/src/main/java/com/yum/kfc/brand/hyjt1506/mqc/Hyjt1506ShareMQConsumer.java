 ...
 *
 */
public class Hyjt1506ShareMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Hyjt1506ShareMQConsumer.class);
	
	private static final String QUEUE_NAME = Hyjt1506Share.class.getCanonicalName();
	
	private static final Hyjt1506Service service = SpringUtil.getBean(Hyjt1506Service.class);
	
	public Hyjt1506ShareMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.hyjt1506.share", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Hyjt1506Share share = JSON.parseObject(String.valueOf(msg.getPayload()), Hyjt1506Share.class);
