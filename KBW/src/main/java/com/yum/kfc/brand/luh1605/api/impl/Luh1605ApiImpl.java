...
import com.yum.kfc.brand.luh1605.api.Luh1605Api;
...
@Path("/luh1605")
@Scope("singleton")
public class Luh1605ApiImpl extends BaseApiImpl implements Luh1605Api {
	
	private static Logger logger = LoggerFactory.getLogger(Luh1605ApiImpl.class);
	
	public static final String ASK_ENC_KEY = "$@_+&%hP";
	public static final String BROWSER_TOKEN_KEY = "@#!@8/&*^";
	public static final int SALT_LENGTH = 6;
	private final String CACHE_KEY_PREFIX = "KFC.LUH1605.";
	...
	private static final Integer AWARD_TYPE = Integer.valueOf(ApplicationConfig.getProperty("campaign.awardType", "41"));
	
	@Autowired
	private MemCachedClient memcachedClient;

	@Autowired
	private Luh1605Service service;

	@Override
	public Result open(Luh1605Parameter parameter, HttpServletRequest request) {
		...
		//检查是否存在iBeacon设备号
		if(null == parameter.getIbeaconSN()){
			return new Result(ApiErrorCode.DEVICE_UUID_ISNULL, "iBeaconSN is empty");
		}
		//检查openId
		if(null == parameter.getOpenId()){
			return new Result(ApiErrorCode.USERID_ISNULL, "openId is empty");
		}
		final Luh1605Open open = new Luh1605Open();
		...
		if (RabbitMQHelper.RABBIT_MQ_ENABLED){
			...
			RabbitMQHelper.publish(Luh1605Open.class.getCanonicalName(), msg, RabbitMQHelper.DEFAULT_PUBLISH_TIMEOUT_MS, new FailPublishHandler(){
				@Override
	...
	@Override
	public Result draw(Luh1605Parameter parameter, HttpServletRequest request){
		parameter.setIpAddr(WebUtil.getRealRemoteAddr(request));
		//检查渠道类型是否正确
		if(null == parameter.getChannelType()){
			return new Result(ApiErrorCode.CHINNEL_TYPE_ISNULL, "Channel Type is empty");
		...
		final Luh1605Draw draw = new Luh1605Draw();
		...
		try{	
			...
			draw.setWxOpenId(parameter.getOpenId());
			...
			if(won){
				...
				if (RabbitMQHelper.RABBIT_MQ_ENABLED){
					...
					RabbitMQHelper.publish(Luh1605Draw.class.getCanonicalName(), msg, RabbitMQHelper.DEFAULT_PUBLISH_TIMEOUT_MS, new FailPublishHandler(){
						@Override
						public void handleFailPublish(Throwable reason, Message msg) {
							logger.warn("failed to publish to MQ, {}, turn to save to DB directly",  reason.getMessage(), reason);
							try{
...
//		addOrIncr(CACHE_KEY_USERCOUNT+parameter.getChannelType()+"-"+parameter.getUserId(), 1, midNight);
	...
	private KBSResultWin requestAwardFromKBS(Luh1605Parameter param) {
		Map<String, Object> p = new HashMap<String, Object>();
