...
public class Luh1605DrawMQConsumer extends BaseRabbitMQConsumer{
	private static final Logger log = LoggerFactory.getLogger(Luh1605DrawMQConsumer.class);
	
	private static final String QUEUE_NAME = Luh1605Draw.class.getCanonicalName();
	
	private static final Luh1605Service service = SpringUtil.getBean(Luh1605Service.class);
	
	public Luh1605DrawMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.luh1605.draw", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Luh1605Draw draw = JSON.parseObject(String.valueOf(msg.getPayload()), Luh1605Draw.class);
