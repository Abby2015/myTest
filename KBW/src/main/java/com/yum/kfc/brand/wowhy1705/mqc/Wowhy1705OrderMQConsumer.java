...
public class Wowhy1705OrderMQConsumer extends BaseRabbitMQConsumer{
	private static final Logger log = LoggerFactory.getLogger(Wowhy1705OrderMQConsumer.class);
	
	private static final String QUEUE_NAME = Wowhy1705Order.class.getCanonicalName();
	
	private static final Wowhy1705Service service = SpringUtil.getBean(Wowhy1705Service.class);
	
	public Wowhy1705OrderMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.wowhy1705.order", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Wowhy1705Order order = JSON.parseObject(String.valueOf(msg.getPayload()), Wowhy1705Order.class);
