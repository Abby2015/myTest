 ...
 *
 */
public class Wxdgb1508AskMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Wxdgb1508AskMQConsumer.class);
	
	private static final String QUEUE_NAME = Wxdgb1508Ask.class.getCanonicalName();
	
	private static final Wxdgb1508Service service = SpringUtil.getBean(Wxdgb1508Service.class);
	
	public Wxdgb1508AskMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.wxdgb1508.ask", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Wxdgb1508Ask ask = JSON.parseObject(String.valueOf(msg.getPayload()), Wxdgb1508Ask.class);
					service.saveAsk(ask);
