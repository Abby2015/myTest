 ...
 *
 */
public class Xrbj1506UserMQConsumer extends BaseRabbitMQConsumer{

	private static final Logger log = LoggerFactory.getLogger(Xrbj1506UserMQConsumer.class);
	
	private static final String QUEUE_NAME = Xrbj1506User.class.getCanonicalName();
	
	private static final Xrbj1506Service service = SpringUtil.getBean(Xrbj1506Service.class);
	
	public Xrbj1506UserMQConsumer(){
		CONSUMERS = Integer.parseInt(ApplicationConfig.getProperty("rmq.queue.consumers.xrbj1506.user", "1"));
	}
	...
	protected void consume() throws ShutdownSignalException, ConsumerCancelledException, IOException, InterruptedException{
		RabbitMQHelper.consume(QUEUE_NAME, new MessageConsumer(){
			...
			public void consume(Message msg) throws ConsumeException{
				try{
					Xrbj1506User user = JSON.parseObject(String.valueOf(msg.getPayload()), Xrbj1506User.class);
